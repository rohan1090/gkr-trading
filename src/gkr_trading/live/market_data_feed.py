...
# ── yfinance-backed feed (free, no subscription) ────────────────────────


class YFinanceMarketDataFeed:
    """Polls yfinance for market data — free alternative to AlpacaMarketDataFeed.

    Works 24/7 with no API key required.  Returns the same MarketDataEnvelope
    objects that AlpacaMarketDataFeed produces.
    """

    def __init__(
        self,
        equity_tickers: Sequence[str] = ("AAPL", "SPY", "QQQ", "TSLA", "NVDA", "MSFT"),
        poll_interval_sec: float = 15.0,
    ) -> None:
        ...
        try:
            ...
            logging.getLogger(__name__).warning("yfinance not installed")
    ...
    def poll(self) -> List[MarketDataEnvelope]:
        """Poll yfinance for current quotes. Returns list of envelopes."""
        ...
        try:
            ...
            for ticker_sym in self._tickers:
                try:
                    ...
                    env = MarketDataEnvelope(
                        instrument_ref=EquityRef(ticker=ticker_sym.upper()),
                        timestamp_ns=now_ns,
                        last_cents=_dollars_to_cents(last_price),
                        open_cents=_dollars_to_cents(open_price),
                        high_cents=_dollars_to_cents(day_high),
                        low_cents=_dollars_to_cents(day_low),
                        close_cents=_dollars_to_cents(last_price),
                    ...
                    logging.getLogger(__name__).warning(
                        f"YFinanceMarketDataFeed: failed to fetch {ticker_sym}: {exc}"
                    )

        except Exception as exc:
            self._stats.errors += 1
            self._stats.consecutive_failures += 1
            logging.getLogger(__name__).error(f"YFinanceMarketDataFeed poll error: {exc}")
